...
import type { UploadCultureImageResult } from "../model/types";

/**
 * Culture 이미지 업로드 Server Action.
 *
 * 항목 대표 이미지(인재상/복지 등)에 사용한다. 클라이언트에서 FormData(`file`)로 호출.
 *
 * 인가: 세션 클라이언트 `getUser()`(미인증 거부). service_role 금지 —
 *   Storage `culture-images` write 정책이 authenticated 롤을 요구한다.
 * 경로: `culture/{uuid}.{ext}` — 원본 파일명 대신 새 uuid로 생성(충돌·경로주입 방지).
...
export async function uploadCultureImage(
  formData: FormData,
): Promise<UploadCultureImageResult> {
  ...
  const path = `culture/${crypto.randomUUID()}.${ext}`;
